    ...
    for p in group.get_players():
        if p.type==False:
            ...
            if p.firms_offer_accepted==1:
                ...
                if p.firm_effort_level==0.8:
                    p.binary_effort_level="Super"
                if p.firm_effort_level==0.3:
                    p.binary_effort_level="Regular"
                print("player effort level is",p.binary_effort_level)
            ...
            if p.worker_has_accepted_offer==1:
                ...
                if p.worker_effort_level==0.8:
                    p.binary_effort_level="Super"
                if p.worker_effort_level==0.3:
                    p.binary_effort_level="Regular"  
...
class Player(BasePlayer):
    ...
    Q2 = models.IntegerField(label='Question 2: If a worker provides SUPER effort, then what is the cost of effort?')
    Q3 = models.IntegerField(label="Question 3: If a worker receives a wage of 58 ECUs and provides REGULAR effort, then what is the worker's profit for the period?")
    Q4 = models.IntegerField(label="Question 4: If a firm hires a worker for 46 ECUs and the worker provides SUPER effort, then what is the firm's for the period?")
    ...
    worker_effort_level = models.FloatField(initial=0)
    firm_effort_level = models.FloatField(initial=0)
    binary_effort_level=models.StringField(initial="")
    effort_cost = models.FloatField(initial=0)
...
def live_effort(player: Player, data):
    ...
    if data["information_type"]=="effort":
        #print("effort selection recieved",data)
        #print("player.worker_selected_effort=",player.worker_selected_effort)
        if data["id_in_group"]==player.id_in_group and player.worker_selected_effort==False:
            ...
            #print("worker effort recieved on python")
            #return {0: dict(information_type="effort",effort_level=data["effort"],transaction_number=data["transaction_number"])}
        elif data["id_in_group"]==player.id_in_group and player.worker_selected_effort==True:
            print("worker has already selected effort")
        temp_group=player.get_others_in_group()
        for p in temp_group:
            #print("looped over player number ",p.id_in_group)
            if p.transaction_number==player.transaction_number and p.type==0:
                firm=p
                #print("firm is:",firm)
                ...
                #Transactions.create(group=player.group, player=player, firm_)
                #print(Transactions.filter())
                #print("firm's effort level and wage are",firm.firm_effort_level,firm.wage)
                #print("worker's effort level and wage are",player.worker_effort_level,player.wage_paid," this should match the above print")
    #if data["information_type"]=="effort2":
        #print("effort2 recieved",data)
        #if data["transaction_number"]==player.transaction_number and player.type==0:
            #player.firm_effort_level=data["effort"]
...
class Instructions_Quiz(Page):
    ...
    def error_message(player: Player, values):
        ...
        if values["Q2"] != 12:
            ...
            error_occured=True
            hint_string=hint_string+"Hint 2: Look at the cost of effort corresponding to super effort in the Cost of Effort table, "
        if values["Q3"] != 30:
            error_string=error_string+"3 "
            error_occured=True
            hint_string=hint_string+"Hint 3: Worker profit = wage-Cost of Effort-26 = 58-2-26, "
        if values["Q4"] != 64:
            error_string=error_string+"4 "
            error_occured=True
            hint_string=hint_string+"Hint 4: Firm Profit=(126-Wage)xEffort Level Modifier = (126-46)x0.8"
...
class Stage_1(Page):
    ...
    def before_next_page(player: Player, timeout_happened):
        import time
        player.effort_start_time=time.time() 
    @staticmethod
    def js_vars(player: Player):
        group = player.group
        return dict(
...
class Instructions_Stage_1_Pause(Page):
    form_model = 'player'
    @staticmethod
    def is_displayed(player: Player):
        return player.round_number == 1  
class Effort_Selection(Page):
    ...
    @staticmethod
    def is_displayed(player: Player):
        return player.round_number >1 and player.type==1 and player.worker_has_accepted_offer == 1
    @staticmethod
    def js_vars(player: Player):
        group = player.group
        return dict(
...
class Experiment_End(Page):
    ...
    def is_displayed(player: Player):
        return player.round_number == 13
page_sequence = [IRB_Info, Instructions_Quiz, Role_assign_waitpage, Instructions_Quiz_Pause, Stage_1_Pause, Stage_1, Instructions_Stage_1_Pause, Effort_Selection, Effort_Waitpage, Period_Payoffs, Payment_Page, Experiment_End]